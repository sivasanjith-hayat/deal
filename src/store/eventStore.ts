...
export type ProjectorMode = 'waiting' | 'countdown' | 'deal' | 'no_deal' | 'negotiate' | 'leaderboard';
...
export interface EventState {
  ...
  setQaDuration: (seconds: number) => void;
...
export const useEventStore = create<EventState>((set, get) => ({
  ...
  pauseEvent: () => set((s) => ({
    ...
    status: 'paused',
    timerRunning: false,
    eventLog: [...s.eventLog, { id: `e${Date.now()}`, action: 'PAUSE', timestamp: Date.now() }],
  })),
  resumeEvent: () => {
    const state = get();
    if (state.statusBeforePause) {
      set((s) => ({
        status: state.statusBeforePause!,
        timerRunning: s.timerSeconds > 0,
        statusBeforePause: null,
        eventLog: [...s.eventLog, { id: `e${Date.now()}`, action: 'RESUME', timestamp: Date.now() }],
      }));
    }
  },
  endEvent: () => set((s) => ({
  ...
  setQaDuration: (seconds) => set({ qaDuration: seconds }),
